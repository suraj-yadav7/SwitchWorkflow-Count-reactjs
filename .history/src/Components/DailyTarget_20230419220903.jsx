...
const DailyTarget = () => {
        ...
        const targetHandle = (e) =>{
            if(radioBtn === "LCB"){
                let output = parseInt(955-((target.npt) + (target.icq/6)))
                console.log(output)
                ...
                let output = parseInt(310-((parseInt((target.npt/1.548).toFixed())) + parseInt((target.icq/6).toFixed())))
                // console.log(310-((target.npt/1.548) + (target.icq/6)))
                console.log(typeof(parseInt((target.npt/1.548).toFixed())))
                setTarget((prev)=>({...prev,targ:output}))
                console.log("the final reulst",target.targ)
                // setBtnClick(true)
            }
